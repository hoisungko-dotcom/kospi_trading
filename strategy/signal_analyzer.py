...
class SignalAnalyzerKospi:
    ...
    BOX_BLOCKED_SYMBOLS = {"252670", "251340", "114800", "253160"}

    @staticmethod
    def _format_reason(code: str, message: str) -> str:
        return f"[{code}] {message}"

    def _box_gate_reason(
        self,
        symbol: str,
        price_data: Dict,
        strong_market: bool,
        ...
        open_price = float(price_data.get('open', close) or close)
        close_prev = float(price_data.get('close_prev', open_price) or open_price)
        volume = float(price_data.get('volume', 0) or 0)
        avg_volume = float(price_data.get('avg_volume_20', volume) or volume)
        sma5 = float(price_data.get('sma_5', close) or close)
        rsi = float(price_data.get('rsi', 50) or 50)
        atr_pct = float(price_data.get('atr_pct', 0.02) or 0.02)
        ...
        gap_pct = (open_price / close_prev - 1.0) if close_prev > 0 else 0.0

        weak_regime = not (strong_market or selective)
        min_volume_ratio = 0.55 if weak_regime else 0.45
        max_atr = 0.10 if weak_regime else 0.14

        if symbol.startswith(("KSPI", "KDQ")):
            return "box.index_row", "지수 행 제외"
        if symbol in self.BOX_BLOCKED_SYMBOLS:
            return "box.derivative_etf", "파생 ETF 제외"
        if avg_volume > 0 and volume < avg_volume * min_volume_ratio:
            return "box.volume_weak", f"거래량 약함({volume / avg_volume:.2f}x)"
        if return5 > 0.25:
            return "box.return5_overheat", f"5일 급등 과열({return5 * 100:.1f}%)"
        if return20 > 0.60:
            return "box.return20_overheat", f"20일 급등 과열({return20 * 100:.1f}%)"
        if return20 > 0.45 and return5 <= 0.08:
            return "box.late_trend_rollover", f"후기 추세 둔화(20일 {return20 * 100:.1f}%)"
        if return20 > 0.25 and sma5 > 0 and close < sma5 * 0.985:
            return "box.post_run_pullback", f"급등 후 눌림 과심(20일 {return20 * 100:.1f}%)"
        if atr_pct > max_atr:
            return "box.volatility_high", f"변동성 과다(ATR {atr_pct * 100:.1f}%)"
        if rsi >= 82:
            return "box.rsi_overheat", f"RSI 과열({rsi:.0f})"
        if gap_pct > 0.10:
            return "box.open_gap_high", f"시가 갭 과다({gap_pct * 100:.1f}%)"
        return None
    ...
    def _detect_signal_v43(
        ...
        if avg_turnover < min_turnover:
            return 'HOLD', self._format_reason(
                "box.turnover_low",
                f"거래대금 부족({avg_turnover/1e8:.0f}억<50억)",
            )

        gate = self._box_gate_reason(symbol, price_data, strong_market, selective)
        if gate is not None:
            code, message = gate
            price_data['_box_reject_code'] = code
            return 'HOLD', self._format_reason(code, message)

        score = self.calculate_score(symbol, price_data)
        grade = self._grade_v43(score)
        if grade is None:
            return 'HOLD', self._format_reason("box.score_low", f'v4.3점수부족({score:.1f}<65)')

        strategy, reason = self._classify_v43(price_data, strong_market, selective)
        if strategy is None:
            return 'HOLD', self._format_reason("box.pattern_missing", reason)

        if strategy == 'PULLBACK' and grade == 'C':
            return 'HOLD', self._format_reason("box.pullback_c_block", f'v4.3 C-PULLBACK 차단({score:.1f})')

        volume_ratio = volume / avg_volume if avg_volume > 0 else 1.0
        if grade == 'B' and strategy in ('MOMENTUM', 'BREAKOUT') and not (strong_market or selective or volume_ratio >= 2.0):
            return 'HOLD', self._format_reason(
                "box.breakout_confirmation_missing",
                f'v4.3 B-{strategy} 시장/거래량 확인 부족',
            )
    ...
    def detect_signal(
        ...
        try:
            ...
            if avg_turnover < min_turnover:
                ...
                return 'HOLD', self._format_reason(
                    "box.turnover_low",
                    f'거래대금부족({avg_turnover/1e8:.0f}억<50억)',
                )

            gate = self._box_gate_reason(symbol, price_data, strong_market, selective)
            if gate is not None:
                code, message = gate
                logger.debug(f"  {symbol}: BOX GATE → {message}")
                return 'HOLD', self._format_reason(code, message)
            ...
            return 'HOLD', self._format_reason("box.condition_miss", hold_reason)
