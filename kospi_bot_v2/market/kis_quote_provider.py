...
class KISQuoteOnlyProvider(MarketDataProvider):
    ...
    def __init__(self, symbols: tuple[str, ...], lookback: int = 100, request_interval: float = 0.12):
        ...
        if not self.client.data_token:
            raise RuntimeError("KIS quote token unavailable. Check KIS_REAL_APPKEY/KIS_REAL_APPSECRET.")
    ...
    def load_universe_frame(self) -> pd.DataFrame:
        ...
        if not rows:
            raise RuntimeError("No quote data loaded from KIS/yfinance.")
    ...
    def _load_index_proxy_frames(self) -> list[pd.DataFrame]:
        """Build broad-market rows from index ETFs, not from the trade candidate list."""
    ...
    def _synthetic_index_rows(self, frame: pd.DataFrame, symbol: str, name: str) -> pd.DataFrame:
        """Build a quiet regime proxy from the loaded universe.

        This avoids pykrx index calls in the runtime path. The v2 engine only
        needs a broad direction proxy; precise index OHLCV can be plugged in
