...
class SectorMonitor:
    ...
    - update(): 업종 분봉 API → 섹터 모멘텀 갱신 (10분 간격)
    ...
    def __init__(self, api_client, update_interval_sec: int = 600):
        ...
        # 종목→섹터 매핑은 백그라운드에서 빌드 (현재는 KIS API 조회)
    ...
    def _lookup_sector_via_kis(self, symbol: str) -> str | None:
        """KIS 종목 조회 API로 섹터 코드 반환 (없으면 None)."""
        try:
            url = (
    ...
    def _build_stock_sector_map(self):
        ...
        현재 브로커의 섹터 조회 소스(KIS)를 배치 호출해 종목→섹터 코드 매핑을 구성한다.
        ...
        try:
            ...
            for sym in symbols:
                ...
                time.sleep(0.06)   # 현재 구현의 KIS rate-limit 준수

            self._stock_sector = mapping
            self._map_ready    = True
            logger.info(
                f"✅ 섹터 매핑 완료 (현재 구현): {len(mapping)}개 종목 / "
    ...
    def _fetch_momentum(self, sector_code: str) -> float | None:
        """현재 구현의 업종 분봉 5봉 기준 모멘텀 계산 → -0.15 ~ +0.15."""
        try:
    ...
    def get_sector_bonus(self, symbol: str) -> float:
        """종목의 섹터 모멘텀 보너스 (-0.15 ~ +0.15). 매핑 없으면 단건 조회 후 캐시."""
